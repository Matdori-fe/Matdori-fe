...
const WriteJokbo: React.FC<WriteJokboProps> = ({ setTitle, setContent }) => {
	...
	function titleInputHeight(event: React.ChangeEvent<HTMLTextAreaElement>) {
		const element = event.target;
		element.style.height = '36px';
		element.style.height = Math.max(36, element.scrollHeight + 2) + 'px';
	...
	return (
		<>
			...
			<textarea
				className='resize-none w-full border-[1px] border-lightGray rounded-basic pt-[8px] pl-3 pr-3 pb-2 text-[12px] font-Regular h-[36px] scrollbar-hide mt-3 mb-3'
