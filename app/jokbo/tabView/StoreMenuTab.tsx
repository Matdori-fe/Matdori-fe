  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setMenuList(response.data.result[0].menus);
        console.log(response.data.result[0].menus);
      } catch (error) {
        console.log(error);
      }
    };
    ...
    <>
      <div className="mb-[150px] h-auto flex flex-wrap justify-center">
        ...
        <div className="w-full mx-4 flex flex-wrap justify-center">
          {menuList.map(({ name, price, imgUrl }) => {
            return <MenuComponent name={name} price={price} imgUrl={imgUrl} />;
          })}
        </div>
